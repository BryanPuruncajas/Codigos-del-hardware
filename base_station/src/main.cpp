...
void onDataReceive(const uint8_t *mac, const uint8_t *incomingData, int data_len) {
  ...
  if (data_len == sizeof(ReceivedData)) {
    ...
    if (latestReceivedData.flag >= 0 && latestReceivedData.flag < MAX_FLAGS) {
        ...
        // Imprime la telemetria directo en el monitor serial de la base,
        // asi se puede ver en vivo sin necesitar el script de python.
        if (latestReceivedData.flag == 1) {
            Serial.print("Telemetry  height=");
            Serial.print(latestReceivedData.values[0], 2);
            Serial.print("  yaw=");
            Serial.print(latestReceivedData.values[1], 2);
            Serial.print("  nicla_x=");
            Serial.print(latestReceivedData.values[2], 0);
            Serial.print("  nicla_y=");
            Serial.print(latestReceivedData.values[3], 0);
            Serial.print("  nicla_w=");
            Serial.print(latestReceivedData.values[4], 0);
            Serial.print("  nicla_h=");
            Serial.println(latestReceivedData.values[5], 0);
        } else if (latestReceivedData.flag == 2) {
            // Resultado del scan I2C inalambrico (ver i2c_scan_wireless.cpp)
            Serial.print("I2C SCAN (inalambrico) -> direcciones encontradas: ");
            bool any = false;
            for (int i = 0; i < 6; i++) {
                int addr = (int)latestReceivedData.values[i];
                if (addr > 0) {
                    if (any) Serial.print(", ");
                    Serial.print("0x");
            ...
            if (!any) Serial.print("(ninguna)");
            Serial.println();
        } else if (latestReceivedData.flag == 3) {
            // Estado de actuadores: servos + potencia de motores (ver main.cpp del blimp)
            Serial.print("Telemetry-Actuadores  servo1=");
            Serial.print(latestReceivedData.values[0], 1);
            Serial.print("grados  servo2=");
            Serial.print(latestReceivedData.values[1], 1);
            Serial.print("grados  motor1=");
            Serial.print(latestReceivedData.values[2] * 100, 0);
            Serial.print("%  motor2=");
            Serial.print(latestReceivedData.values[3] * 100, 0);
            Serial.println("%");
        } else if (latestReceivedData.flag == 4) {
            // Orientacion completa del IMU (ver main.cpp del blimp)
            Serial.print("Telemetry-Orientacion  roll=");
            Serial.print(latestReceivedData.values[0], 2);
            Serial.print("  pitch=");
            Serial.print(latestReceivedData.values[1], 2);
            Serial.print("  yaw=");
